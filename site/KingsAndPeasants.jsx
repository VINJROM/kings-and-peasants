...
function CreateLobby({ profile, onBack, onEnter, onOpenAudio }) {
  ...
  useEffect(() => {
    ensureNet();
    return netOn((m) => {
      if (m.t === "lobby" && m.lobby) { setBusy(false); onEnter(m.lobby.code); }
      if (m.t === "error") { setBusy(false); showToast(m.msg); }
    });
  }, [onEnter, showToast]);
  const create = () => {
    if (!SERVER_URL) return showToast("Multiplayer isn't configured yet — set the server address in index.html (see README).");
    setBusy(true);
    if (!netSend({ t: "create", settings: { rounds: 3, botSpeed: "fast" } })) { setBusy(false); showToast("Can't reach the game server."); }
  };
  return (
    <div className="wrap">
      <div className="topbar" style={{ paddingLeft: 0 }}>
...
function BrowseLobbies({ profile, onBack, onEnter, onOpenAudio }) {
  ...
  const refresh = useCallback(() => {
    if (!SERVER_URL) { setLobbies([]); return; }
    netSend({ t: "browse" });
  }, []);
  useEffect(() => {
    ensureNet();
    const off = netOn((m) => {
      if (m.t === "lobbies") setLobbies(m.list);
      if (m.t === "lobby" && m.lobby) onEnter(m.lobby.code);
      if (m.t === "error") showToast(m.msg);
      if (m.t === "open") refresh();
    });
    refresh();
    const t = setInterval(refresh, 4000);
    return () => { off(); clearInterval(t); };
  }, [refresh, onEnter, showToast]);

  const join = (c) => {
    if (!SERVER_URL) return showToast("Multiplayer isn't configured yet — set the server address in index.html (see README).");
    netSend({ t: "join", code: c });
  };
      ...
      <div className="card-panel">
        ...
        {lobbies && lobbies.map((l) => (
          <div key={l.code} className="lobbyrowitem">
            <Avatar p={{ name: l.hostName }} />
            <div className="grow">
              <div style={{ fontFamily: "var(--util)", fontWeight: 700 }}>{l.hostName}'s table</div>
              <div className="hint">{l.count}/10 seated · {l.rounds} rounds · code {l.code}</div>
            </div>
...
function LobbyRoom({ profile, code, onBack, onGameStart }) {
  ...
  const [toast, showToast] = useToast();
  const isHost = lobby && lobby.host === profile.id;

  useEffect(() => {
    ensureNet();
    const off = netOn((m) => {
      if (m.t === "lobby" && m.lobby && m.lobby.code === code) setLobby(m.lobby);
      if (m.t === "started" && m.code === code) onGameStart(code);
      if (m.t === "kicked") { showToast("The host removed you from the table."); onBack(); }
      if (m.t === "error") { showToast(m.msg); if (/No lobby/.test(m.msg || "")) onBack(); }
      if (m.t === "open") netSend({ t: "join", code });   // reattach after a reconnect
    });
    netSend({ t: "join", code });                          // idempotent: fetches the current lobby
    return off;
  }, [code, onBack, onGameStart, showToast]);

  const addBot = (diff) => netSend({ t: "addBot", diff });
  const removePlayer = (id) => { const idx = lobby ? lobby.players.findIndex((p) => p.id === id) : -1; if (idx > 0) netSend({ t: "kick", idx }); };
  const setRounds = (r) => netSend({ t: "settings", patch: { rounds: r } });
  const setSpeed = (v) => netSend({ t: "settings", patch: { botSpeed: v } });
  const leave = () => { netSend({ t: "leave" }); onBack(); };
  const start = () => {
    if (!lobby || lobby.players.length < 2) return showToast("You need at least 2 players.");
    netSend({ t: "start" });
  };
          ...
          <div key={p.id} className="lobbyrowitem">
            ...
            <div className="grow">
              <div style={{ fontFamily: "var(--util)", fontWeight: 700 }}>
                {p.name} {p.id === lobby.host && <span style={{ color: "var(--gilt)" }}>· host</span>} {p.id === profile.id && <span style={{ color: "var(--smoke)" }}>· you</span>}
              </div>
...
/* ============================================================ multiplayer net (Phase 2)
   A single WebSocket to the game server. The server owns all truth: it applies every
   action through the shared engine, drives bots and clocks, and pushes personalized
   state (other players' hands arrive redacted as zeros). Configure the address by
   setting window.KP_SERVER (see index.html) — e.g. "wss://your-app.up.railway.app". */
const SERVER_URL = (typeof window !== "undefined" && window.KP_SERVER) || null;
let _net = null;
let _netProfile = null;
const netListeners = new Set();
function netOn(fn) { netListeners.add(fn); return () => netListeners.delete(fn); }
function netProfile(p) { _netProfile = { id: p.id, name: p.name, avatar: p.avatar || null, royal: p.royal || "majesty" };
  if (_net && _net.ws.readyState === 1) _net.ws.send(JSON.stringify({ t: "hello", profile: _netProfile })); }
function ensureNet() {
  if (!SERVER_URL || typeof WebSocket === "undefined") return null;
  if (_net && _net.ws.readyState <= 1) return _net;
  const ws = new WebSocket(SERVER_URL);
  const net = { ws, queue: [] };
  ws.onopen = () => {
    if (_netProfile) ws.send(JSON.stringify({ t: "hello", profile: _netProfile }));
    net.queue.splice(0).forEach((m) => ws.send(JSON.stringify(m)));
    for (const fn of [...netListeners]) fn({ t: "open" });
  };
  ws.onmessage = (ev) => { let m; try { m = JSON.parse(ev.data); } catch { return; } for (const fn of [...netListeners]) fn(m); };
  ws.onclose = () => { for (const fn of [...netListeners]) fn({ t: "closed" }); setTimeout(() => { if (_net === net) { _net = null; if (netListeners.size) ensureNet(); } }, 1500); };
  _net = net;
  return net;
}
function netSend(msg) {
  const n = ensureNet(); if (!n) return false;
  if (n.ws.readyState === 1) n.ws.send(JSON.stringify(msg)); else n.queue.push(msg);
  return true;
}
...
export function GameTable({ profile, mode, initial, code, sfxVol = 1, haptics = true, onOpenAudio, onExit }) {
  ...
  // ---- online sync: the server pushes personalized state; the client only sends intents
  useEffect(() => {
    if (!online) return;
    ensureNet();
    const off = netOn((m) => {
      if (m.t === "state" && m.st && m.st.code === code) {
        if (!stRef.current || m.st.version >= stRef.current.version) { setSt(m.st); setSel(null); }
      }
      if (m.t === "error" && m.msg) showToast(m.msg);
      if (m.t === "open") netSend({ t: "join", code });         // reconnect: reattach and refetch
    });
    netSend({ t: "join", code });                               // fetch current state on mount (idempotent)
    return off;
  }, [online, code, showToast]);

  const commit = useCallback((ns) => { setSt(ns); setSel(null); }, []);   // local matches only
  ...
  // ---- bot loop
  useEffect(() => {
    if (online) return;                                 // the server drives bots in online games
  ...
  useEffect(() => {
    ...
    const to = setTimeout(() => {
      const cur = stRef.current;
      if (cur && cur.version === ver && cur.phase === "play" && !online) commit(applyAction(cur, { type: "pass" }));
    }, 5000);
  ...
  useEffect(() => {
    return;                                             // retired: the server's move clock covers absent players
  ...
  useEffect(() => {
    if (online) return;                                 // the server runs taxation timers online
  ...
  const doPlay = () => {
    ...
    if (online) { netSend({ t: "action", action: { type: "play", ...sel } }); setSel(null); }
    else commit(applyAction(st, { type: "play", ...sel }));
  };
  const doPass = () => {
    if (!(myTurn && (st.trick || mustPass))) return;
    if (online) netSend({ t: "action", action: { type: "pass" } });
    else commit(applyAction(st, { type: "pass" }));
  };
  const nextRound = () => { if (online) netSend({ t: "nextRound" }); else commit(startRound(st)); };
    ...
    <div className="game">
      <div className="gamehead">
        <button className="back" style={{ background: "none", color: "var(--gilt)", fontWeight: 700 }}
          onClick={() => { if (!online && st.phase !== "matchEnd") setConfirmExit(true); else { if (online) netSend({ t: "leave" }); onExit(); } }}>‹ Exit</button>
          ...
          <div className="sheet">
            ...
            {st.revolt.pIdx === myIdx ? (
              <>
                ...
                <button className="btn gold" style={{ textAlign: "center", marginTop: 16 }} onClick={() => online ? netSend({ t: "revolt", accept: true }) : commit(resolveRevolt(st, true))}>
                  <span className="t">{st.revolt.seat === n - 1 ? "⚔️ Overturn the court" : "⚔️ Declare revolution"}</span>
                </button>
                <button className="btn" style={{ textAlign: "center", marginTop: 10 }} onClick={() => online ? netSend({ t: "revolt", accept: false }) : commit(resolveRevolt(st, false))}>
      ...
      {st.phase === "tax" && st.tax && (() => {
        ...
        if (payIdx >= 0) return <PeasantPayer key={"pay" + st.round + "-" + payIdx} st={st} entryIdx={payIdx}
          onConfirm={(cards) => online ? netSend({ t: "taxPay", cards }) : commit(resolveTaxPayment(st, payIdx, cards))} />;
        ...
        if (mine >= 0) return <TaxChooser key={"ret" + st.round + "-" + mine} st={st} entryIdx={mine}
          onConfirm={(cards) => online ? netSend({ t: "taxReturn", cards }) : commit(resolveTaxReturn(st, mine, cards))} />;
        return (
          <div className="overlay">
            <div className="sheet">
              <h2>Taxation</h2>
...
export default function App() {
  ...
  useEffect(() => { if (profile) netProfile(profile); }, [profile]);
